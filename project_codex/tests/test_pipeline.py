...
def test_step1_extract_terms(workflow_engine, mock_llm, mock_rag):
    ...
    suggestions, metadata = workflow_engine.step1_extract_terms("He cast Fireball.")
...
def test_step1_extract_terms_known_term(workflow_engine, mock_llm, mock_rag):
    ...
    suggestions, _ = workflow_engine.step1_extract_terms("He cast Fireball.")
...
def test_step2_draft(workflow_engine, mock_llm, mock_rag):
    ...
    draft, metadata = workflow_engine.step2_draft("Source text")
...
def test_step3_judge(workflow_engine, mock_llm):
    mock_llm.call_llm.return_value = (json.dumps({"issues": []}), {"tokens": 50, "cost": 0.05})

    critique, metadata = workflow_engine.step3_judge("Draft", "Context")
...
def test_step4_polish(workflow_engine, mock_llm, mock_rag):
    ...
    final, metadata = workflow_engine.step4_polish("Draft", {"critique": "ok"})
